...
@pytest.mark.asyncio
async def test_arewrite_content(
    mock_system_message,
    ...
    await service.arewrite_content(content_rewrite_req)
    ...
    mock_llm_service.return_value.get_chat_runnable.assert_called_once_with(
        llm_id=content_rewrite_req.llm_id,
        temperature=content_rewrite_req.temperature,
    )
    mock_str_output_parser.assert_called_once()


def test_generate_prompt_returns_string_with_request_fields(service, content_rewrite_req):
