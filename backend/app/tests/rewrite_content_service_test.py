...
@patch.object(RewriteContentService, '_generate_prompt')
@patch('app.services.llm_service.LLMService.get_chat_runnable')
async def test_arewrite_content_no_temperature(mock_get_chat_runnable, mock_generate_prompt):
    mock_db = MagicMock()
    service = RewriteContentService(mock_db)
    ...
    mock_generate_prompt.assert_called_once_with(mock_request)
    mock_get_chat_runnable.assert_called_once_with(model='mock_model', temperature=float(Config.DEFAULT_TEMPERATURE))
    assert isinstance(result, AsyncIterable)
...
async def test_arewrite_content_no_model(mock_get_chat_runnable, mock_generate_prompt):
    ...
    mock_generate_prompt.assert_called_once_with(mock_request)
    mock_get_chat_runnable.assert_called_once_with(model=Config.DEFAULT_LANGUAGE_TRANSLATION_MODEL, temperature=0.5)
    assert isinstance(result, AsyncIterable)

@pytest.mark.asyncio
@patch.object(RewriteContentService, '_generate_prompt')
@patch('app.services.llm_service.LLMService.get_chat_runnable')
async def test_arewrite_content_different_prompt(mock_get_chat_runnable, mock_generate_prompt):
    mock_db = MagicMock()
    service = RewriteContentService(mock_db)
